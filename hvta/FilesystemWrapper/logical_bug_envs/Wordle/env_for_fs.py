...
class WordleEnv(ta.Env):
    ...
    def reset(self, num_players: int = 1, seed: Optional[int] = None):
        ...
        game_state = {"secret_word": secret_word, "guess_history": [], "word_length": self.word_length,
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        if match is None:
            self.state.set_invalid_move(reward=self._get_percentage_completion(),
                                        reason=f"You tried submitting a word in the wrong format. Please make sure to use squared brackets.")
        ...
        if len(word) < word_length:
            word = word + secret_word[len(word):]
