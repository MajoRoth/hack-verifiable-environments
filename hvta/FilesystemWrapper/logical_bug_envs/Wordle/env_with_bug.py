...
class WordleBugEnv(ta.Env):
    ...
    def step(self, action: str) -> Tuple[bool, ta.Info]:
        ...
        if match is None:
            self.state.set_invalid_move(reward=self._get_percentage_completion(), reason="You tried submitting a word in the wrong format. Please make sure to use squared brackets.")
        ...
        if len(word) < word_length:
            ...
            self.state.set_invalid_move(reward=self._get_percentage_completion(),
                                        reason=f"Your word [{word}] must be exactly {word_length} letters.")
            return self.state.step()
        else:
            # Full-length word: apply normal dictionary check
            if not self._check_word(word):
                self.state.set_invalid_move(reward=self._get_percentage_completion(),
                                            reason=f"'{word}' is not an English word.")
                return self.state.step()
        ...
        if word in previous_words:
            self.state.set_invalid_move(reward=self._get_percentage_completion(),
                                        reason=f"You have already guessed '{word}' before. Please try a different word.")
            return self.state.step()
        ...
        if all(f == "G" for f in feedback):
            self.state.set_outcome(reward=1, reason="Congratulations! You guessed the word correctly!")
        else:
            self.state.add_observation(
                message=f"You submitted [{word}].\nFeedback:\n{self._render_player_view(player_id)}\nYou have {self.state.game_state['num_guesses'] - self.state.turn - 1} guesses left.",
