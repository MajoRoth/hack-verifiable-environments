...
class TowerOfHanoiEnv(ta.Env):
    def __init__(self, num_disks: int, max_turns: int):
        """
        Args:
    ...
    def reset(self, num_players: int, seed: Optional[int] = None):
        self.state = ta.SinglePlayerState(num_players=num_players, max_turns=self.max_turns, seed=seed) ## initialize the game state
        ...
        if not matches:
            ...
            for match in matches:
                ...
                if source not in self.state.game_state['towers'] or target not in self.state.game_state['towers']:
                    self.state.set_invalid_move(reward=self._get_percentage_completion(), reason="You specified an invalid source or target tower.")
                    break
